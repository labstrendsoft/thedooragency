  ...
  const onSubmit = (data: ContactFormData) => {
    startTransition(async () => {
      try {
        ...
        setResult({ success: false, error: String(error ?? 'Error desconocido') });
      }
    });
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem>
                <FormControl>
                  <Input
                    placeholder="Nombre"
                    {...field}
                    className="text-thedooragency-negro border-[#161A31] bg-[#F0F0F0] placeholder:text-gray-500 focus-visible:border-gray-600 focus-visible:ring-0"
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
            ...
            <FormItem>
              <FormControl>
                <Textarea
                  placeholder="Mensaje"
                  {...field}
                  className="text-thedooragency-negro min-h-[120px] border-[#161A31] bg-[#F0F0F0] placeholder:text-gray-500 focus-visible:border-gray-600 focus-visible:ring-0"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
        ...
        <button
          ...
          className={`w-full cursor-pointer rounded-md py-1.5 text-white ${
            isPending
              ...
              : 'bg-thedooragency-negro hover:bg-gray-900'
          }`}
        >
          {isPending ? (
            <>
        ...
        {result && (
          <p className={`mt-2 text-center ${result.success ? 'text-green-600' : 'text-red-600'}`}>
            {result.success
              ? 'Correo enviado ✅'
