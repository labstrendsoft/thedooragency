    ...
    <Accordion type="single" collapsible className="w-full space-y-4 px-6" defaultValue="item-1">
      {services.map((service) => (
        <AccordionItem key={service.id} value={service.id} className="border-none">
          <AccordionTrigger className="bg-thedooragency-negro cursor-pointer rounded-none px-6 py-6 hover:no-underline">
            <div className="flex max-w-[950px] items-start gap-4 lg:mx-auto lg:items-center">
              <div className="flex flex-col gap-3">
                <h3 className="text-xl font-black text-white lg:text-4xl">{service.title}</h3>
                <h4 className="text-xs text-white lg:text-base">{service.subtitle}</h4>
          ...
          <AccordionContent>
            <div className="flex flex-col items-center gap-10 pt-8 lg:flex-row">
              <Image
